...
std::vector<Vector2D> PhysicsEngine::simulateTrajectory(Vector2D start, Vector2D target, double wind, double mass, double launchSpeed) {
    ...
    for (int i = 0; i <= steps; ++i) {
        ...
        double windEffect = (wind / mass) * (d / 10.0);
        
        double x = start.getX() + d + windEffect;
        double y = start.getY() - gravityDrop; 
...
bool PhysicsEngine::checkCollision(Vector2D objPos, Vector2D targetPos, double radius) {
    return objPos.distanta(targetPos) <= radius;
}